...
class AddAnimalScreen extends StatefulWidget {
  ...
  @override
  State<AddAnimalScreen> createState() => _AgregarAnimalScreenState();
}

class _AgregarAnimalScreenState extends State<AddAnimalScreen>
  ...
  Widget _buildProduccionTab() {
    return ListView(
      ...
      padding: const EdgeInsets.all(16),
      children: [const ProductionFormSection(), const SizedBox(height: 80)],
    );
  }
  ...
  Future<void> _guardarAnimal() async {
    ...
    if (!_formController.isFormValid) {
      if (!_formController.sinigaIsValid) {
        ...
        _navegarATab(0);
      } else if (_formController.nfcId == null ||
          _formController.nfcId!.isEmpty) {
    ...
    try {
      ...
      final existente = await isarService.getAnimalByNfc(
        _formController.nfcId!,
      );
      if (existente != null) {
