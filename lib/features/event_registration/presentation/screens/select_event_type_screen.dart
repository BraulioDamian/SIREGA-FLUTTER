...
import 'package:sirega_app/features/event_registration/presentation/screens/event_data_screen.dart';
...
class SelectEventTypeScreen extends StatelessWidget {
        ...
        children: [
          ...
          for (int i = 0; i < _eventTypes.length; i += 2)
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(child: _buildCard(context, _eventTypes[i])),
                  const SizedBox(width: 16),
                  if (i + 1 < _eventTypes.length)
                    Expanded(child: _buildCard(context, _eventTypes[i + 1]))
                  else
  ...
  Widget _buildCard(BuildContext context, TipoEvento eventType) {
    ...
    return _EventTypeCard(
      ...
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => EventDataScreen(type: eventType)),
      ),
    );
  }
  ...
  static const _eventTypeDetails = <TipoEvento, _EventDetails>{
    ...
    TipoEvento.tratamiento: _EventDetails(
      ...
      'Tratamiento Médico',
      'Antibióticos o medicación general',
      AppColors.info,
    ),
