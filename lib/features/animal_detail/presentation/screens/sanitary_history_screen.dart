...
class _SanitaryHistoryScreenState extends State<SanitaryHistoryScreen>
  ...
  late Future<List<MedicalEventRecord>> _dataFuture;

  @override
  void initState() {
    super.initState();
    final initialIndex = _tabs.indexWhere(
      (t) => t.label == widget.initialFilter,
    );
    ...
    _dataFuture = RepositoryProvider.of<IsarService>(
      context,
  ...
  List<MedicalEventRecord> _filterEvents(
    List<MedicalEventRecord> all,
    TipoEvento? tipo,
  ) {
    final list = tipo == null
        ? List.of(all)
        : all.where((e) => e.tipo == tipo).toList();
    list.sort((a, b) => b.date.compareTo(a.date));
    return list;
  }

  Map<String, List<MedicalEventRecord>> _groupByMonth(
    List<MedicalEventRecord> events,
  ) {
    final map = <String, List<MedicalEventRecord>>{};
    for (final e in events) {
      final key = DateFormat('MMMM yyyy', 'es').format(e.date);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<MedicalEventRecord>>(
        future: _dataFuture,
        builder: (context, snapshot) {
          ...
          return TabBarView(
            ...
            children: [
              for (final t in _tabs)
                _EventListPage(
                  events: _filterEvents(allEventos, t.tipo),
...
class _EventListPage extends StatelessWidget {
  ...
  final Map<String, List<MedicalEventRecord>> Function(List<MedicalEventRecord>)
  groupByMonth;
  final _TabDef tab;
  ...
  Widget build(BuildContext context) {
    ...
    if (tab.tipo == TipoEvento.vacuna) {
      final conRefuerzo = events
          .where((e) => e.nextApplicationDate != null)
          .toList();
      final unicaAplicacion = events
          .where((e) => e.nextApplicationDate == null)
          .toList();
      ...
      return CustomScrollView(
        ...
        slivers: [
          SliverToBoxAdapter(child: _buildSummaryBar(events)),
  ...
  List<Widget> _buildGroupedSlivers(List<MedicalEventRecord> events) {
    final grouped = groupByMonth(events);
    return [
  ...
  Widget _buildSummaryBar(List<MedicalEventRecord> events) {
    final lastDate = events.first.date;
