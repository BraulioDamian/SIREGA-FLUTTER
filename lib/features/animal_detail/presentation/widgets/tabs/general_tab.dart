...
class GeneralTab extends StatelessWidget {
        ...
        child: Column(
          children: [
            AnimalDetailHelpers.buildDetailRow(
              context,
              'No. de Arete',
              animal.displaySiniigaId.isNotEmpty
            ...
            AnimalDetailHelpers.buildDetailRow(
              context,
              'Sexo',
              animal.sex.name.toUpperCase(),
              animal.sex.name == 'macho' ? Icons.male : Icons.female,
              valueColor: animal.sex.color,
            ),
            AnimalDetailHelpers.buildDetailRow(
              context,
              'Fecha de Nacimiento',
              AnimalDetailHelpers.formatDate(animal.birthDate),
              Icons.cake,
            ),
            AnimalDetailHelpers.buildDetailRow(
              context,
              'Edad',
              animal.getFormattedAge(),
      ...
      AnimatedInfoCard(
        ...
        child: Column(
          children: [
            ...
            if (animal.visualTagId == null && animal.rfidTagId == null)
      ...
      AnimatedInfoCard(
        ...
        child: Column(
          children: [
            ...
            if (animal.offspring.isNotEmpty) ...[
              if (animal.dam.value != null || animal.sire.value != null)
            ...
            if (animal.dam.value == null &&
                animal.sire.value == null &&
                animal.offspring.isEmpty)
              AnimalDetailHelpers.buildEmptyState(
  ...
  Widget _buildParentRow(
    ...
    final name = parent.name.isNotEmpty ? parent.name : 'Sin name';
    final fotoPath = parent.profilePhotoUrl ?? parent.photoUrl;
    final hasPhoto =
        fotoPath != null && fotoPath.isNotEmpty && File(fotoPath).existsSync();

    // Calcular edad
    final now = DateTime.now();
    final diff = now.difference(parent.birthDate);
    ...
    // Info chips
    final chips = <String>[
      if (parent.breed.isNotEmpty) parent.breed,
      edad,
      if (parent.visualTagId != null && parent.visualTagId!.isNotEmpty)
        parent.visualTagId!,
    ];
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: !hasPhoto ? Icon(icon, size: 26, color: color) : null,
            ),
  ...
  List<Widget> _buildCriasRows(BuildContext context) {
    final offspring = animal.offspring.toList()
      ..sort((a, b) => b.birthDate.compareTo(a.birthDate));
    return offspring
        .map(
          (cria) => _buildParentRow(
            context,
            cria.sex == Sexo.macho ? 'Hijo' : 'Hija',
            cria,
            cria.sex == Sexo.macho ? Icons.male : Icons.female,
            Colors.brown,
          ),
        )
        .toList();
