...
class VacunaInfo {
  ...
  /// Calcula la fecha de próxima aplicación a partir de la fecha dada.
  DateTime? proximaAplicacion(DateTime fechaAplicacion) {
    if (esAplicacionUnica || intervaloDias == null) return null;
    return fechaAplicacion.add(Duration(days: intervaloDias!));
  }

  /// Texto legible del intervalo de aplicación.
  String get intervaloTexto {
    if (esAplicacionUnica) return 'Aplicación única';
    if (intervaloDias == null) return 'Según indicación veterinaria';
    if (intervaloDias! >= 365) {
      final anios = intervaloDias! ~/ 365;
      return anios == 1 ? 'Cada año' : 'Cada $anios años';
    }
    if (intervaloDias! >= 30) {
      final meses = intervaloDias! ~/ 30;
      return meses == 1 ? 'Cada mes' : 'Cada $meses meses';
    }
    return 'Cada $intervaloDias días';
  }
}
...
class CatalogoVacunas {
  ...
  static const derriengue = VacunaInfo(
    nombre: 'Derriengue (Rabia Paralítica)',
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Campaña SENASICA. Prevención de rabia paralítica bovina transmitida por murciélagos vampiro (Desmodus rotundus). Obligatoria en zonas endémicas.',
  ...
  static const clostridiosis = VacunaInfo(
    ...
    esAplicacionUnica: false,
    intervaloDias: 180,
    descripcion: 'Polivalente 7/8 vías contra Clostridium. Previene pierna negra, edema maligno, enterotoxemia, hepatitis necrótica. Primer refuerzo 21 días después de la primera dosis.',
    color: Color(0xFF9C27B0),
    edadRecomendada: 'A partir de 2-3 meses. Refuerzo a los 21 días, luego cada 6 meses.',
  );
  ...
  static const leptospirosis = VacunaInfo(
    ...
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Prevención de leptospirosis (5 serovares). Causa abortos, problemas renales y baja en producción. Más importante en temporada de lluvias y zonas húmedas.',
    color: Color(0xFF009688),
    edadRecomendada: 'A partir de 4 meses. Refuerzo anual, idealmente antes de lluvias.',
  );
  ...
  static const complejoRespiratorio = VacunaInfo(
    nombre: 'Complejo Respiratorio (IBR+DVB+PI3+BRSV)',
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Vacuna combinada contra Rinotraqueítis Infecciosa Bovina (IBR), Diarrea Viral Bovina (DVB), Parainfluenza 3 (PI3) y Virus Sincitial Respiratorio Bovino (BRSV).',
    color: Color(0xFF4CAF50),
    edadRecomendada: 'A partir de 3-6 meses. Refuerzo a los 21 días, luego anual.',
  );
  ...
  static const carbunco = VacunaInfo(
    nombre: 'Carbunco Bacteridiano (Ántrax)',
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Prevención de Ántrax (Bacillus anthracis). Obligatoria en zonas endémicas del trópico mexicano. Vacuna de cepa Sterne.',
  ...
  static const anaplasmosis = VacunaInfo(
    ...
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Prevención de Anaplasma marginale, transmitida por garrapatas. Causa anemia severa y muerte súbita. Común en trópico húmedo de México.',
    color: Color(0xFF795548),
    edadRecomendada: 'Becerros de 3-9 meses (mayor resistencia natural). Refuerzo anual.',
  );
  ...
  static const babesiosis = VacunaInfo(
    ...
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Prevención de Babesia bovis y B. bigemina, transmitidas por garrapata Rhipicephalus (Boophilus) microplus. Endémica en trópico mexicano.',
    color: Color(0xFF3F51B5),
    edadRecomendada: 'Becerros de 4-10 meses. Refuerzo anual en zonas endémicas.',
  );
  ...
  static const pasteurelosis = VacunaInfo(
    nombre: 'Pasteurelosis (Mannheimiosis)',
    esAplicacionUnica: false,
    intervaloDias: 180,
    descripcion: 'Prevención de Mannheimia haemolytica y Pasteurella multocida. Importante en ganado de engorda, corrales de acopio y destetes estresantes.',
    color: Color(0xFF00BCD4),
    edadRecomendada: 'Al ingreso a corral/engorda o al destete. Refuerzo cada 6 meses.',
  );
  ...
  static const diarreaNeonatal = VacunaInfo(
    nombre: 'Diarrea Neonatal (Scours)',
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Rotavirus, Coronavirus y E. coli. Se aplica a vacas gestantes (60 y 30 días antes del parto) para proteger al becerro vía calostro. No protege a la vaca directamente.',
    color: Color(0xFF8D6E63),
    edadRecomendada: 'Vacas gestantes: 60 y 30 días antes del parto. Refuerzo cada gestación.',
  );
  ...
  static const campylobacteriosis = VacunaInfo(
    nombre: 'Campylobacteriosis (Vibriosis)',
    esAplicacionUnica: false,
    intervaloDias: 365,
    descripcion: 'Prevención de Campylobacter fetus (enfermedad venérea bovina). Causa infertilidad y abortos tempranos. Se aplica 30-60 días antes del empadre o inseminación. Disponible en combo Lepto-Vibrio.',
    color: Color(0xFF5C6BC0),
    edadRecomendada: '30-60 días antes de temporada de empadre/inseminación. Refuerzo anual.',
  );
  ...
  static const brucelosisCepa19 = VacunaInfo(
    nombre: 'Brucelosis (Cepa 19)',
    esAplicacionUnica: true,
    descripcion: 'Campaña obligatoria SENASICA (NOM-041-ZOO-1995). Cepa clásica S19 para becerras. Dosis única de por vida. Solo hembras.',
  ...
  static const brucelosisRB51 = VacunaInfo(
    nombre: 'Brucelosis (Cepa RB51)',
    esAplicacionUnica: true,
    descripcion: 'Campaña SENASICA. Cepa RB51 para hembras adultas no vacunadas previamente con Cepa 19. No interfiere con pruebas serológicas. Dosis única.',
  ...
  static const tuberculina = VacunaInfo(
    nombre: 'Tuberculina (Prueba diagnóstica)',
    esAplicacionUnica: true,
    descripcion: 'Prueba de tuberculina (PPD bovino). NO es vacuna, es prueba diagnóstica de la Campaña contra Tuberculosis Bovina SENASICA (NOM-031-ZOO-1995). Se registra para trazabilidad.',
  ...
  static const List<VacunaInfo> todas = [
    ...periodicas,
    ...unicas,
  ];

  /// Busca una vacuna por nombre en el catalogo.
  /// Retorna null si no se encuentra (vacuna personalizada).
  static VacunaInfo? buscarPorNombre(String nombre) {
    final nombreLower = nombre.toLowerCase().trim();
    for (final vacuna in todas) {
      if (vacuna.nombre.toLowerCase() == nombreLower) return vacuna;
    }
    // Mapeo de nombres legacy/alternativos para compatibilidad
    if (nombreLower.contains('derriengue') || nombreLower == 'rabia' || nombreLower.contains('rabia paralitica')) {
      return derriengue;
    }
    if (nombreLower.contains('clostridi') || nombreLower.contains('pierna negra')) {
      return clostridiosis;
    }
    if (nombreLower.contains('leptospir')) {
      return leptospirosis;
    }
    if (nombreLower.contains('ibr') || nombreLower.contains('dvb') || nombreLower.contains('respiratorio') || nombreLower.contains('pi3')) {
    ...
    if (nombreLower.contains('diarrea neonatal') || nombreLower.contains('scours') || nombreLower.contains('rotavirus')) {
      return diarreaNeonatal;
    }
    if (nombreLower.contains('campylobacter') || nombreLower.contains('vibrio') || nombreLower.contains('lepto-vibrio')) {
  ...
  static Set<String> get nombresEstandar =>
      todas.map((v) => v.nombre).toSet();
}
