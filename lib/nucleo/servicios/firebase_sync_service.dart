...
import 'package:sirega_app/nucleo/servicios/sync_mappers.dart';
...
class FirebaseSyncService {
  ...
  Future<void> _handleRemoteAnimalChange(DocumentSnapshot doc) async {
    await IsarService.isar.writeTxn(() async {
      await SyncMappers.createOrUpdateLocalAnimal(doc.id, doc.data() as Map<String, dynamic>);
    });
  }

  Future<void> _handleRemoteEventChange(DocumentSnapshot doc) async {
    await IsarService.isar.writeTxn(() async {
      await SyncMappers.createOrUpdateLocalEvent(doc.id, doc.data() as Map<String, dynamic>);
    });
  }

  Future<void> _handleRemoteProductionChange(DocumentSnapshot doc) async {
    await IsarService.isar.writeTxn(() async {
      await SyncMappers.createOrUpdateLocalProduction(doc.id, doc.data() as Map<String, dynamic>);
    });
  }

  Future<void> _handleRemoteHerdChange(DocumentSnapshot doc) async {
    await IsarService.isar.writeTxn(() async {
      await SyncMappers.createOrUpdateLocalHerd(doc.id, doc.data() as Map<String, dynamic>);
    });
  }

  Future<void> _handleRemoteBatchEventChange(DocumentSnapshot doc) async {
    await IsarService.isar.writeTxn(() async {
      await SyncMappers.createOrUpdateLocalBatchEvent(doc.id, doc.data() as Map<String, dynamic>);
    });
  }
