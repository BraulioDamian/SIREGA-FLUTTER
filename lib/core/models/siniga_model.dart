...
class SinigaId {
  ...
  static Future<Map<String, String>> _cargarEstados() async {
    ...
    try {
      ...
      for (var estado in estadosJson) {
        _estadosCache![estado['clave']] = estado['name'];
      }
  ...
  // Método sincrónico para obtener name del estado (si ya se cargó el JSON)
  String? get nombreEstadoSync {
    if (estadoClave == null || _estadosCache == null) return null;
    return _estadosCache![estadoClave];
  }

  // Método asíncrono para obtener name del estado
  ...
  static Future<List<EstadoMexico>> getEstadosParaDropdown() async {
    ...
    return estados.entries
        .map((entry) => EstadoMexico(clave: entry.key, name: entry.value))
        .toList()
